...
tasklist = (function ($, m, host, io, storage) {
	...
	var _private = {
		initLocalDb: function initLocalDb() {
		...
		renderList: function renderList(data) {
			...
			if (data.length == 0) data = [{ "id": 0, "text": "No tasks here. Yay!" }];
		...
		getUserLocation: function getUserLocation() {
			if (m.geolocation) {
				navigator.geolocation.getCurrentPosition(function (p) {
					...
					var map = new google.maps.Map(_mapEle, myOptions);
		...
		autoSaveInput: function autoSaveInput(e) {
		...
		loadAutoSaveState: function loadAutoSaveState() {
			var ele = $eleTxtTask;
	...
	var api = {
		saveTask: function saveTask(e, opts) {
			var txt = $eleTxtTask,
			    newTask = null;
		...
		deleteTask: function deleteTask(e, opts) {
			var key = null;
		...
		assignTaskTo: function assignTaskTo(taskId, newUser) {
			//Get the task item
		...
		getTaskById: function getTaskById(taskId, successCallback) {
			storage.getTaskById(taskId, function (task) {
				successCallback(task);
			});
		},

		updateTask: function updateTask(task, successCallback, transactionCallback) {
			storage.updateTask(task, function (t) {
				...
				if (transactionCallback !== undefined) {
					transactionCallback();
				}
			});
		},
		...
		init: function init(eleBtnSave, eleListTask, eleTxtTask) {
